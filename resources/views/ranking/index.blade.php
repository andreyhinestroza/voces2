...
<style>
    ...
    width: 160px;             /* 👈 círculo grande para acomodar font-size 120 */
    height: 160px;
    border-radius: 50%;       /* 👈 círculo perfecto */
    background-color: rgba(64, 119, 122, 0.18); /* 👈 más transparente */
    color: #fff;              /* 👈 número en blanco */
    display: flex;
    align-items: center;
    justify-content: center;
    font-size: 120px;         /* 👈 número grande */
    font-weight: bold;
    z-index: 2;
...
<div class="animated-bg min-h-screen py-12 mt-24">
    ...
    <div style="display:flex; justify-content:center; gap:2rem; flex-wrap:wrap;">
        @foreach($ranking->take(5) as $index => $item)
            <div class="ranking-card {{ $index < 3 ? 'scale-110' : '' }}"
                ...
                <div class="ranking-content p-6">
                    ...
                    <p class="text-sm text-gray-700 mb-2"><strong>Participante:</strong> {{ ucwords($item->participante) }}</p>
                    <span class="inline-block bg-[#FAC00B] text-black px-3 py-1 rounded-full font-bold">{{ $item->votos }} votos</span>
                </div>
            </div>
        @endforeach
    </div>
...
<!-- Tipografía institucional (Montserrat) -->
<link href="https://fonts.googleapis.com/css2?family=Montserrat:wght@400;500;700&display=swap" rel="stylesheet">

<style>

    /* Tarjeta de título institucional */ 
    .section-title-card { width: 75%; margin: 1rem auto; /* 👈 menos espacio arriba y abajo */ background: linear-gradient(90deg, #004d3f, #00A06E, #004d3f); border-radius: 20px; box-shadow: 0 12px 32px rgba(0,0,0,0.35); padding: 30px 24px; /* 👈 menos espacio interno arriba/abajo */ text-align: center; font-family: 'Montserrat', sans-serif; color: #ffffff; }    .section-title-icon { width: 100px; /* más grande */ height: 100px; border-radius: 50%; background: #FAC00B; /* dorado institucional */ color: #000000; display: flex; align-items: center; justify-content: center; font-size: 56px; /* icono grande */ font-weight: 700; margin: 0 auto 20px auto; box-shadow: 0 6px 18px rgba(250,192,11,0.45); /* brillo dorado */ } .section-title-text { font-size: 2.8rem; /* equivalente a text-5xl */ font-weight: 700; margin-bottom: 12px; text-shadow: 2px 2px 4px rgba(0,0,0,0.25); /* sombra en letras */ } .section-title-sub { font-size: 1.3rem; font-weight: 500; opacity: 0.95; }
    .section-title-text { color: #ffffffff; font-size: 2.5rem; /* equivalente a text-4xl */ 
    font-weight: 700; text-shadow: 1px 1px 2px rgba(243, 241, 241, 1); 
    margin-bottom: 8px; } 
    .section-title-sub { color: #ffffff; /* letras blancas */ font-size: 1.1rem; /* tamaño */ font-weight: 700; /* 👈 ahora en negrilla */ opacity: 0.85; /* ligera transparencia */ }

    /* Botones de concursos */ 
    .concursos-buttons { max-width: 960px; margin: 0 auto; display: flex; 
    flex-wrap: wrap; gap: 12px; 
    justify-content: center; } 
    .concurso-btn { font-family: 'Montserrat', sans-serif; 
    padding: 10px 18px; 
    border-radius: 999px; 
   ...
   .ranking-concursos-card {
    max-width: 960px;
    margin: 1rem auto; /* menos espacio arriba y abajo */
    background: linear-gradient(90deg, #004d3f, #00A06E, #004d3f); /* degradado institucional */
    border-radius: 16px;
    box-shadow: 0 12px 32px rgba(0,0,0,0.35); /* sombra más fuerte */
    border: none; /* quitamos borde sólido para que luzca el degradado */
    padding: 40px 24px; /* 👈 conservamos tu tamaño */
    text-align: center;
    font-family: 'Montserrat', sans-serif;
...
.ranking-concursos-icon {
    width: 64px;   /* 👈 conservamos tamaño */
    height: 64px;
    border-radius: 50%;
    background: #FAC00B; /* dorado institucional */
    color: #000000;
    display: flex;
    align-items: center;
    justify-content: center;
    font-size: 32px; /* 👈 conservamos tamaño */
    font-weight: 700;
    margin: 0 auto 16px auto;
    box-shadow: 0 4px 12px rgba(250,192,11,0.45); /* brillo dorado */
}

.ranking-concursos-title {
    color: #ffffff; /* ahora blanco sobre fondo verde */
    font-size: 32px; /* 👈 conservamos tamaño */
    font-weight: 700;
    margin-bottom: 8px;
...
.ranking-concursos-subtitle {
    color: #ffffff; /* blanco */
    font-size: 18px; /* 👈 conservamos tamaño */
    font-weight: 700; /* negrilla */
    opacity: 0.95; /* un poco más visible */
}
